...
export default function ResumePreview({ data, template = "professional" }: ResumePreviewProps) {
  ...
  // Creative Template (Modern, space-efficient two-column A4 layout)
  const CreativeTemplate = () => (
    ...
    >
      {/* Sidebar */}
      <div className="w-1/4 min-w-[180px] max-w-[220px] bg-gradient-to-b from-purple-600 to-blue-500 text-white p-6 flex flex-col gap-7" style={{boxSizing: 'border-box'}}>
        {/* Profile */}
        <div className="text-center mb-3">
          <div className="w-16 h-16 rounded-full bg-white/20 mx-auto mb-3 flex items-center justify-center text-2xl font-bold border-2 border-white/30">
            {data.personalInfo.fullName ? getInitials(data.personalInfo.fullName) : "YN"}
          </div>
          <h1 className="text-lg font-bold leading-tight mb-1 truncate">{data.personalInfo.fullName || "Your Name"}</h1>
          <p className="text-xs font-medium bg-white/10 px-2 py-0.5 rounded-full inline-block mb-1 truncate">{jobTitle}</p>
        </div>
        {/* Contact */}
        <div>
          <h2 className="text-xs font-semibold uppercase tracking-wider border-b border-white/20 pb-1 mb-2">Contact</h2>
          <div className="space-y-1 text-xs">
            {data.personalInfo.email && (
              <div className="flex items-center gap-2"><Mail className="w-3 h-3 text-yellow-200" />{data.personalInfo.email}</div>
            )}
            {data.personalInfo.phone && (
              <div className="flex items-center gap-2"><Phone className="w-3 h-3 text-yellow-200" />{data.personalInfo.phone}</div>
            )}
            {data.personalInfo.location && (
              <div className="flex items-center gap-2"><MapPin className="w-3 h-3 text-yellow-200" />{data.personalInfo.location}</div>
            )}
            {data.personalInfo.website && (
              <div className="flex items-center gap-2"><Globe className="w-3 h-3 text-yellow-200" />{data.personalInfo.website}</div>
            )}
            {data.personalInfo.linkedin && (
              <div className="flex items-center gap-2"><Linkedin className="w-3 h-3 text-yellow-200" />{data.personalInfo.linkedin}</div>
            )}
          </div>
        </div>
        {/* Skills */}
        {data.skills.length > 0 && (
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-wider border-b border-white/20 pb-1 mb-2">Skills</h2>
            <div className="flex flex-col gap-2">
              {data.skills.map((skill) => (
                <div key={skill.id}>
                  <div className="font-semibold text-xs mb-1">{skill.category}</div>
        ...
        {data.certifications.length > 0 && (
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-wider border-b border-white/20 pb-1 mb-2">Certifications</h2>
            <div className="flex flex-col gap-2">
              {data.certifications.map((cert) => (
                <div key={cert.id} className="flex flex-col gap-0.5">
                  <span className="font-semibold text-xs flex items-center gap-1"><Award className="w-3 h-3 text-yellow-200" />{cert.name}</span>
                  {cert.issuer && <span className="text-[10px] text-white/80">{cert.issuer}</span>}
                  {cert.date && <span className="text-[10px] text-white/60">{formatDate(cert.date)}</span>}
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Main Content */}
      <div className="flex-1 p-7 flex flex-col gap-4 overflow-y-auto" style={{boxSizing: 'border-box'}}>
        {/* Summary */}
        {data.summary && (
          <div>
            <h2 className="text-sm font-bold text-purple-700 mb-1 uppercase tracking-wide">Profile</h2>
            <p className="text-xs text-gray-800 leading-snug">{data.summary}</p>
          </div>
        )}
        {/* Experience */}
        {data.experience.length > 0 && (
          <div>
            <h2 className="text-sm font-bold text-purple-700 mb-1 uppercase tracking-wide">Experience</h2>
            <div className="flex flex-col gap-2">
              {data.experience.map((exp) => (
                <div key={exp.id}>
                  <div className="flex justify-between items-baseline mb-0.5">
                    <div>
                      <div className="font-semibold text-xs text-gray-900 leading-tight">{exp.position}</div>
                      <div className="text-xs text-purple-700 font-medium">
                        {exp.company}
                        {exp.location && `, ${exp.location}`}
                      </div>
                    </div>
                    <div className="text-[10px] text-gray-500 font-light whitespace-nowrap ml-2">
                      {formatDate(exp.startDate)} - {exp.current ? "Present" : formatDate(exp.endDate)}
                    </div>
                  </div>
                  {exp.description && (
                    <div className="text-xs leading-snug text-gray-700 mt-0.5">
                      {exp.description.split("\n").map((line, idx) => (
        ...
        {data.education.length > 0 && (
          <div>
            <h2 className="text-sm font-bold text-purple-700 mb-1 uppercase tracking-wide">Education</h2>
            <div className="flex flex-col gap-2">
              {data.education.map((edu) => (
                <div key={edu.id}>
                  <div className="flex justify-between items-baseline mb-0.5">
                    <div>
                      <div className="font-semibold text-xs text-gray-900 leading-tight">{edu.degree}</div>
                      <div className="text-xs text-purple-700 font-medium">
                        {edu.institution}
                        {edu.location && `, ${edu.location}`}
                      </div>
        ...
        {data.projects.length > 0 && (
          <div>
            <h2 className="text-sm font-bold text-purple-700 mb-1 uppercase tracking-wide">Projects</h2>
            <div className="flex flex-col gap-2">
              {data.projects.map((project) => (
                <div key={project.id}>
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {project.link && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-[10px] text-purple-700 hover:underline flex items-center gap-1"
                      >
                        <ExternalLink className="h-3 w-3" />
                  ...
                  {project.description && (
                    <div className="text-xs text-gray-700 mb-0.5">{project.description}</div>
                  )}
                  {project.technologies && (
                    <div className="flex flex-wrap gap-1 mt-0.5">
                      {project.technologies.split(",").map((tech, idx) => (
                        <span key={idx} className="text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full">
                          {tech.trim()}
                        </span>
                      ))}
                    </div>
  ...
  const MinimalTemplate = () => (
    ...
    >
      ...
      <div className="w-80 bg-gray-50 p-8 flex flex-col gap-6 border-r border-gray-200">
        {/* Profile Section */}
        <div className="text-center">
          <div className="w-32 h-32 rounded-full bg-gray-200 mx-auto mb-4 flex items-center justify-center text-3xl font-light text-gray-600 border border-gray-300">
            {data.personalInfo.fullName ? getInitials(data.personalInfo.fullName) : "YN"}
          </div>
